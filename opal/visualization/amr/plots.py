...
def line_plot(ds, axis, field, **kwargs):
    """
    Plot a line plot of 3D data along an axis
    
    Parameters
    ----------
    ds      (AmrDataset)    datasets
    ...
    if not ds.filetype == FileType.AMR:
        raise TypeError(ds.filename + ' is not an AMR dataset.')
    ...
    c = ds.real_ds.find_max(field)[1]
    ray = ds.real_ds.ortho_ray(ax, (c[cut1], c[cut2]))
...
def slice_plot(ds, normal, field, **kwargs):
    """
    ...
    slc = yt.SlicePlot(ds.real_ds, normal=normal,
...
def projection_plot(ds, axis, field, **kwargs):
    """
    ...
    slc = yt.ProjectionPlot(ds.real_ds, axis, fields=field,
...
def particle_plot(ds, x_field, y_field, z_field=None, **kwargs):
    """
    ...
    pp = yt.ParticlePlot(ds.real_ds, x_field, y_field, z_field,
...
def particle_phase_space_plot(ds, axis, **kwargs):
    """
    Plot particle phase spaces etc of 3D data
    ...
    pp = yt.ParticlePlot(ds.real_ds, coordinate, momentum,
                         fontsize=fontsize,
